 ...
 */
public class CeilQuad implements Drawable {
	public final static short	WALL_ALIGNS_MAP_Y	= 0;
	public final static short	WALL_ALIGNS_MAP_X	= 1;

	public short[]	data = new short[36];
	public final static short DATA_ALIGN = 0;
	public final static short DATA_ANGLE_WALL = 4;
	public final static short DATA_X_WORLD = 8;
	public final static short DATA_Z_WORLD = 12;
	public final static short DATA_LENGTH_WORLD = 16;
	public final static short DATA_LENGTH_LEFT_CORRECTION = 20;
	public final static short DATA_LENGTH_RIGHT_CORRECTION = 24;
	public final static short DATA_TEXTURE_SPACE_SIN = 28;
	public final static short DATA_TEXTURE_SPACE_COS = 32;
...
//	public int[]			textureSpaceCos	= new int[4]; 
	...
	public CeilQuad( int x1, int y1, int z1,
						int x2, int y2, int z2,
						int x3, int y3, int z3,
						int x4, int y4, int z4, Renderer renderer) {
	
		data[DATA_X_WORLD+0] = (short) x1; //data[DATA_X_WORLD+0] = x1; 
		...
		for( int vertice=0; vertice<4; vertice++ ) {
			...
			data[DATA_TEXTURE_SPACE_SIN+vertice] 			= (short) (renderer.sin(((int)data[DATA_ANGLE_WALL+vertice]))>>16); 
			data[DATA_TEXTURE_SPACE_COS+vertice] 			= (short) (renderer.cos((int)data[DATA_ANGLE_WALL+vertice])>>16); 

			data[DATA_LENGTH_WORLD+vertice] 				= (short) (Float.sqrt( new Float(xDiff*xDiff + zDiff*zDiff) ).toLong());
			data[DATA_LENGTH_LEFT_CORRECTION+vertice] 	= (short) (((int)-data[DATA_LENGTH_WORLD+vertice])/100);	
...
//System.out.println( "x_from_left[num_vectors_left]: " +  x_from_left[num_vectors_left-1] + " y_from_left[num_vectors_left]: " + y_from_left[num_vectors_left-1] );			
//System.out.println( "x_to_left[num_vectors_left]: " +  x_to_left[num_vectors_left-1] + " y_to_left[num_vectors_left]: " + y_to_left[num_vectors_left-1] );			
//System.out.println( "z_to_left[num_vectors_left]: " +  z_to_left[num_vectors_left-1] );			
...
//System.out.println( "x_from_right[num_vectors_right]: " +  x_from_right[num_vectors_right-1] + " y_from_right[num_vectors_right]: " + y_from_right[num_vectors_right-1] );			
//System.out.println( "x_to_right[num_vectors_right]: " +  x_to_right[num_vectors_right-1] + " y_to_right[num_vectors_right]: " + y_to_right[num_vectors_right-1] );			
//System.out.println( "z_to_right[num_vectors_right]: " +  z_to_right[num_vectors_right-1] );			
		...
		// Very late y_top clip.... Must be optimized to front...
		...
		if( y_bottom > y_top ) {
			...
			int renderer_width_f	= y_bottom*scanwidth_f + ((int)this.renderer_width_f);
			...
			int offset_y			= y_bottom*scanwidth_f;
...
//OPTIMIZE!!!
			...
			for( int row=y_bottom; row>=y_top; row-- ) {
...
//System.out.println( "-->currDxdy_left: " +  currDxdy_left );			
...
//System.out.println( "-->x_from_left[currVectorIdx_left]: " +  x_from_left[currVectorIdx_left] );			
					currVectorIdx_left++;
...
//System.out.println( "-->row: " + row + " curr_y_from_left " + curr_y_from_left  );			
//System.out.println( "-->curr_y_to_left " + curr_y_to_left  );			
...
//					int yx_left_local  = (poly_x_left_f >>16);
//					int yx_right_local = (poly_x_right_f>>16);
					int yx_left_local;//  = (poly_x_left_f >>16);// + row*scanwidth;
					int yx_right_local;// = (poly_x_right_f>>16);// + row*scanwidth;
					...
					int poly_u_center_f = (cam_x_f + z_ray_center * cos_center); 
					int poly_v_center_f = (cam_z_f + z_ray_center * sin_center);
					
//					int width_neg 		= ((poly_x_left_f-renderer_width_f)>>16);
					int width_neg 		= yx_left_local-(renderer_width_f>>16);
					long texelsPerPoly_f	= (dt_y_bottom_inv_f + ((z_ray_center-z_ray_y_bottom)*d_dt_f));
					int du_f 			= (int)  ((texelsPerPoly_f * sin_center)>>16); // cant be optimized due precision
					int dv_f 			= (int) -((texelsPerPoly_f * cos_center)>>16);
					int poly_u_left_f	= poly_u_center_f + du_f * width_neg;
					int poly_v_left_f	= poly_v_center_f + dv_f * width_neg;
...
//System.out.println( "-->poly_x_left_f " + ((poly_x_left_f>>16)%scanwidth)  );			
//System.out.println( "-->poly_x_right_f " + ((poly_x_right_f>>16)%scanwidth)  );			
