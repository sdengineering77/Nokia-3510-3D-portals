...
//		}
	...
	public void setClipArea( int x1, int y1, 
		...
		for( int columnCnt=x1; columnCnt<=x2; columnCnt++ ) {
...
/*		int currX;
		...
		xy_left    = scanwidth*y1;
		if( y2 > y1 ) {
			dx_f 		= (int) (((long) width)<<16) / (y2 - y1);
			xy_right = scanwidth*y2;
			currX_f = x1_f;
			prevX   = x1;
			for( int xy=xy_left; xy<=xy_right; xy+=scanwidth ) {
				...
				for( int columnCnt=prevX; columnCnt<currX; columnCnt++ ) {
					viewportY_top_array[columnCnt] 	= xy + columnCnt;				
				}
				prevX = currX;
			}
		}	else
		if( y2 < y1 ) {
			dx_f 		= (int) (((long) width)<<16) / (y1 - y2);
			xy_right = scanwidth*y2;
			currX_f = x1_f;
			prevX   = x1;
			for( int xy=xy_left; xy>=xy_right; xy-=scanwidth ) {
			...
			for( int columnCnt=x1; columnCnt<=x2; columnCnt++ ) {
				viewportY_top_array[columnCnt] 	= xy_left + columnCnt;				
			}
		}

		xy_left  = scanwidth*y3;
		if( y4 > y3 ) {
			dx_f	= (int) (((long) width)<<16) / (y4 - y3);
		...
		if( y4 < y3 ) {
			...
			for( int columnCnt=x1; columnCnt<=x2; columnCnt++ ) {
				viewportY_bottom_array[columnCnt] 	= xy_left + columnCnt;				
			}
		}
*/
