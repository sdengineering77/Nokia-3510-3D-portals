...
public class ViewportWallQuad implements Drawable {
	...
	public final static int	num_clipAreas		= 20;
				...
				if( useTexture ) {	
					...
					for( int columnCnt=x_left; columnCnt<x_right; columnCnt++ ) {
						...
						for( int rowCnt=y_top_local; rowCnt<y_bottom_local; rowCnt+=scanwidth) {
							...
							int   buff_r    	= (buff_argb&0x0000FF00);
							int   buff_g    	= (buff_argb&0x0000F0F0);
							int   buff_b    	= (buff_argb&0x0000F00F);
							int   new_r     	= buff_r + (((((text_argb&0x00000F00) - buff_r) * textureAlpha)>>4)&0x00000F00);
							int   new_g     	= buff_g + (((((text_argb&0x000000F0) - buff_g) * textureAlpha)>>4)&0x000000F0);
							int   new_b     	= buff_b + (((((text_argb&0x0000000F) - buff_b) * textureAlpha)>>4)&0x0000000F);
