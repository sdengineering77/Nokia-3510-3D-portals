...
//System.out.println( "z_to_right[num_vectors_right]: " +  z_to_right[num_vectors_right-1] );			
		...
		if( y_bottom > y_top ) {
			...
			if( num_vectors_left > 0 ) {
				...
				if( clipped ) {
//					currVectorIdx_left++;
					while( curr_y_to_left > y_bottom && (currVectorIdx_left+1) < num_vectors_left ) {
...
//					poly_x_left_f += currDxdy_left*diff;
					poly_x_left_f  = curr_y_to_left*scanwidth_f + (x_to_left[currVectorIdx_left]<<16) + currDxdy_left*(curr_y_to_left-viewportY_bottom);
				}	else {
			...
			if( num_vectors_right > 0 ) {
				...
				if( clipped ) {
//					currVectorIdx_right++;
					while( curr_y_to_right > y_bottom && (currVectorIdx_right+1) < num_vectors_right ) {
...
//					currVectorIdx_right--;
					...
					poly_x_right_f  = curr_y_to_right*scanwidth_f + (x_to_right[currVectorIdx_right]<<16) + currDxdy_right*(curr_y_to_right-viewportY_bottom);
			...
			for( int row=y_bottom; row!=y_top; ) {
				...
				for( ;row!=endRow; row-- ) {
					...
					if( poly_x_right_f > poly_x_left_f ) {
						...
						int poly_u_left_f		= du_f * width_neg + (z_ray_center * cos_center + cam_x_f);
						int poly_v_left_f		= dv_f * width_neg + (z_ray_center * sin_center + cam_z_f);
